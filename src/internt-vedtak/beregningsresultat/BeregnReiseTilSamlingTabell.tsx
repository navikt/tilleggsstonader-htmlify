...
export const BeregnReiseTilSamlingTabell: React.FC<{
    ...
    const offentligTransportSamling = beregningsresultatReiseTilSamling.offentligTransport ?? [];
    const privatBilSamling = beregningsresultatReiseTilSamling.privatBil ?? [];

    if (!offentligTransportSamling) return '';
    if (!privatBilSamling) return '';

    return (
        <>
            {offentligTransportSamling.map((samling, reiseIndex) => {
                return (
            ...
            {privatBilSamling.map((samling, reiseIndex) => {
                return (
                    <div key={reiseIndex} style={{ marginBottom: '2rem' }}>
                        <h3>{samling.adresse}</h3>
                        <table style={{ fontSize: '75%' }}>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    {samling.fergekostnad != null && (
                                        <td>{samling.fergekostnad}</td>
                                    )}
